...
def _fit_direct_correlated_error_model(sample):
    sigma_y = np.asarray(sample["sigma_log_sfr"], dtype=float)
    sigma_z = np.asarray(sample["sigma_oh"], dtype=float)

    finite = np.isfinite(sigma_y) & np.isfinite(sigma_z)
    if np.sum(finite) < 2:
        raise RuntimeError("At least two finite propagated-error pairs are required to fit the error-error relation.")

    x_obs = sigma_y[finite]
    y_obs = sigma_z[finite]

    if np.nanstd(x_obs) == 0.0:
        slope = 0.0
        intercept = float(np.nanmedian(y_obs))
    else:
        slope, intercept = np.polyfit(x_obs, y_obs, deg=1)

    positive_obs = y_obs[y_obs > 0.0]
    floor = float(np.nanmin(positive_obs)) if positive_obs.size else float(np.finfo(float).eps)
    raw_fit = slope * x_obs + intercept

    return {
        "slope": float(slope),
        "intercept": float(intercept),
        "floor": floor,
        "n_clipped": int(np.sum(raw_fit < floor)),
        "observed_sigma_log_sfr": x_obs,
        "observed_sigma_oh": y_obs,
    }


def _apply_direct_correlated_error_model(sigma_log_sfr, error_model):
    sigma_y = np.asarray(sigma_log_sfr, dtype=float)
    sigma_z = error_model["slope"] * sigma_y + error_model["intercept"]
    return np.maximum(sigma_z, error_model["floor"])


def _apply_scaled_direct_correlated_error_model(scaled_sigma_log_sfr, error_model, error_scale=1.0):
    sigma_y_scaled = np.asarray(scaled_sigma_log_sfr, dtype=float)
    scale = float(error_scale)
    if scale <= 0.0:
        raise ValueError("error_scale must be positive.")

    sigma_y_base = sigma_y_scaled / scale
    sigma_z_base = _apply_direct_correlated_error_model(sigma_y_base, error_model)
    return sigma_z_base * scale


def _plot_smoothed_density_contour(
    ax,
    x_vals,
    y_vals,
    x_range,
    ...
    x = np.asarray(x_vals[finite], dtype=float)
    y = np.asarray(y_vals[finite], dtype=float)
    ...
    level_idx = np.searchsorted(H_cumsum, coverage * H_cumsum[-1])
    level = H_flat[min(level_idx, H_flat.size - 1)]
...
def run_sec34_style_null_test(
    ...
    error_model = _fit_direct_correlated_error_model(sample)
    ...
    sigma_y_sim_base = sigma_y.copy()
    sigma_z_sim_base = _apply_direct_correlated_error_model(sigma_y_sim_base, error_model)
    sigma_y_sim = sigma_y_sim_base * error_scale
    sigma_z_sim = sigma_z_sim_base * error_scale
    ...
    for i in range(n_realizations):
        ...
        y_mock = y_true + rng.normal(loc=0.0, scale=sigma_y_sim)
        z_mock = z_true + rng.normal(loc=0.0, scale=sigma_z_sim)
    ...
    return {
        ...
        "primary_model": model,
        "direct_error_model": {
            "slope": error_model["slope"],
            "intercept": error_model["intercept"],
            "floor": error_model["floor"],
            "n_clipped": error_model["n_clipped"],
            "error_scale": error_scale,
            "observed_sigma_log_sfr": error_model["observed_sigma_log_sfr"],
            "observed_sigma_oh": error_model["observed_sigma_oh"],
            "simulated_sigma_log_sfr_base": sigma_y_sim_base,
            "simulated_sigma_oh_base": sigma_z_sim_base,
            "simulated_sigma_log_sfr": sigma_y_sim,
            "simulated_sigma_oh": sigma_z_sim,
        },
        "null": {
...
def print_sec34_summary(result):
    ...
    if error_model is not None:
        error_scale = result.get("config", {}).get("error_scale", 1.0)
        print(
            "Direct correlated error fit: "
            f"sigma([O/H]) = {error_model['intercept']:.6f} + "
            f"{error_model['slope']:.6f} * sigma(log Sigma_SFR)"
        )
        print(f"Simulated error scale factor: x{error_scale:g}")
        if error_model["n_clipped"] > 0:
            print(
                f"Applied a positive sigma([O/H]) floor to {error_model['n_clipped']} "
...
def plot_sec34_style_null_test(result, reference_curve=None, panel_mode="full"):
    ...
    if panel_mode in {"full", "rho_and_error"} and error_model is not None:
        ...
        sim_sigma_y = np.asarray(error_model["simulated_sigma_log_sfr"], dtype=float)
        sim_sigma_z = np.asarray(error_model["simulated_sigma_oh"], dtype=float)
        ...
        error_ax.scatter(
            ...
            color="0.45",
            edgecolors="none",
            label="Observed propagated errors",
        )
        ...
        error_ax.scatter(
            ...
            alpha=0.18,
            color="tab:orange",
            label="Simulated direct-correlated errors",
        )
...
def generate_sec34_mock_realization(
    ...
    return {
        ...
        "sigma_log_sfr": sigma_y_sim,
        "sigma_oh": sigma_z_sim,
        "direct_error_model": {
            "slope": error_model["slope"],
            "intercept": error_model["intercept"],
            "floor": error_model["floor"],
